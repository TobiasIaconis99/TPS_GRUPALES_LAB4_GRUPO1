...
public class ServletCuenta extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("agregar".equals(accion)) {
			try {
				...
				TipoCuenta tipoCuenta = tipoCuentaNegocio.obtenerTipoCuentaPorId(idTipoCuenta); // Obtener el objeto TipoCuenta completo
				
				if (tipoCuenta == null) {
					System.out.println("ServletCuenta: ERROR - Tipo de cuenta NO encontrado para ID: " + idTipoCuenta); // Debug
					session.setAttribute("mensajeError", "Tipo de cuenta no encontrado o inválido.");
					response.sendRedirect("ServletCuenta?accion=listar");
					return;
				} else {
					System.out.println("ServletCuenta: Tipo de cuenta encontrado: " + tipoCuenta.getNombreTipoCuenta()); // Debug
				}
				...
				if (cliente == null) {
					...
					System.out.println("ServletCuenta: Cliente encontrado: " + cliente.getDni() + " - " + cliente.getNombre()); // Debug
				}
				...
				System.out.println("ServletCuenta: Cuenta a agregar - DNI: " + cliente.getDni() + 
				                   ", Tipo ID: " + tipoCuenta.getIdTipoCuenta() + 
				                   ", Nro Cuenta: " + numeroCuentaGenerado + 
				                   ", CBU: " + cbuGenerado); // Debug
				...
				System.err.println("ServletCuenta: NumberFormatException al convertir el ID de Tipo Cuenta: " + e.getMessage()); // Debug
				e.printStackTrace(); // Imprimir el stack trace completo
				session.setAttribute("mensajeError", "Error en el formato del tipo de cuenta. Seleccione un tipo válido.");
			} catch (Exception e) {
	...
	// Métodos auxiliares para generar números de cuenta y CBU (pueden ser más sofisticados)
    private String generateUniqueAccountNumber() {
        // Genera un número de cuenta simple para propósitos de prueba
        // En un entorno real, esto debería ser más robusto y verificar unicidad en la DB
