...
public class CuentaDaoImpl implements CuentaDao {
	...
	private static final String obtenerCuentasPorClienteId = "SELECT c.idCuenta, c.numeroCuenta, c.cbu, c.saldo, c.fechaCreacion, c.estado AS estadoCuenta, "
			+ "t.idTipoCuenta, t.nombreTipoCuenta, t.estado AS estadoTipoCuenta " + "FROM Cuenta c "
			+ "INNER JOIN TipoCuenta t ON c.idTipoCuenta = t.idTipoCuenta " + "WHERE c.idCliente = ?";
	...
	public boolean agregar(Cuenta cuenta) {
		...
		try {
			...
			statement.setString(3, cuenta.getNumeroCuenta()); // Se espera que ya venga generado desde la capa de
																// Negocio
			statement.setString(4, cuenta.getCbu()); // Se espera que ya venga generado desde la capa de Negocio
			statement.setBigDecimal(5, cuenta.getSaldo()); // Se espera que el saldo inicial ya venga desde la capa de
															// Negocio
			...
			// Considerar relanzar una excepción personalizada o devolver un código de error
			// más específico
		} finally {
	...
	public String generarSiguienteCBU() {
		...
		BigInteger valorInicialCBU = new BigInteger("10000000000000000000"); // Asegura que este sea un valor inicial
																				// adecuado
	...
	public Cuenta obtenerCuentaPorCBU(String cbu) {
		...
		try {
			...
			statement = conexion.prepareStatement(
					"SELECT idCuenta, idCliente, idTipoCuenta, numeroCuenta, cbu, saldo, fechaCreacion, estado FROM Cuenta WHERE cbu = ? AND estado = 1");
			statement.setString(1, cbu);
			resultSet = statement.executeQuery();
	...
	public boolean descontarSaldo(int idCuenta, BigDecimal monto) {
		...
		try {
			...
			e.printStackTrace();
			System.err.println("Error al descontar saldo de la cuenta: " + e.getMessage());
		} finally {
			closeResources(conexion, statement, null);
		}
